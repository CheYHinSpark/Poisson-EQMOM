...
tsp = 1000;
RM = zeros(N+1,nY,nX); % m0,m1,m2,...,mN
...
for i = 1:nX
    for j = 1:nY
        M = RM(1:N+1,j,i);
        [rho(:,j,i), phi(:,j,i), r(j,i), shift(j,i) ] = Invn(M);
    end
end
...
while( tmoment<tsp - 1e-6 )
    ...
    for j = 1:nY
        flux_X(:,j,1) = F_int_l(3:N+3,j,1) + F_int_l(1:N+1,j,1) + F_int_r(3:N+3,j,nX) + F_int_r(1:N+1,j,nX);
        flux_X(:,j,nX+1) = F_int_l(3:N+3,j,1) + F_int_l(1:N+1,j,1) + F_int_r(3:N+3,j,nX) + F_int_r(1:N+1,j,nX);
    end
...
name = strcat(datestr(datetime,'yyyymmdd'), "_Vicsek_N", num2str(N), "_X", num2str(nX), ".mat");
save(name);
